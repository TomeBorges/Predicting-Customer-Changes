...
- Looking at the size of the transactions: (2 transactions were for 0 items, this should be cleaned), 2163 transactions were for just 1 item, 1647 transactions for 2 items, all the way up to the biggest transaction: 1 transaction for 30 items. This indicates that most customers buy a small number of items in each transaction.
-Additionally the size of purchases seems right skewed (as the mean is larger than the median an as can be seen below in the barplot of the table of transaction sizes);


```{r - Top 20 items purchased by frequency}
...
```
Using only 1 variable and a linear regression the best results I obtained in forecasting the Volume was with the PositiveServiceReview feature. The RMSE is 688 and the R-squared is 0.33, which are two extremely high values revealing that this model is not effective and is not robust or accurate enough to be utilized with confidence.
...
plot_train_df %>% 
  filter(ProductType %in% c("PC", "Laptop", "Netbook","Smartphone")) %>%
  group_by(ProductType) %>%
  summarize(Volume = sum(Volume), x5star)

dcast(plot_train_df, ProductType ~ x5StarReviews, value.var = "Volume", fun.aggregate=sum)
```
...
asd <- plot_train_df %>%
  group_by(ProductType) %>%
  summarize(VolumeTotal= sum(Volume), 
            x5Star = sum(x5StarReviews),
            x4Star = sum(x4StarReviews),
            x3Star = sum(x3StarReviews),
            x2Star = sum(x2StarReviews),
            x1Star = sum(x1StarReviews),
...
asd<- dcast(melt(asd), variable ~ProductType , value.var = "value", fun.aggregarte='sum')
asd
```
