...
```
Using only 1 variable and a linear regression the best results I obtained in forecasting the Volume was with the PositiveServiceReview feature. The RMSE is 672 and the R-squared is 0.38, which are two extremely high values revealing that this model is not effective and is not robust or accurate enough to be utilized with confidence.
...
The value of the Root Mean Squared Error is ~198, taking into account that the average Volume is 700, this error is not terrible, but it is not great as well. The same goes for a Mean Absolute Error of ~152.
The rsquared is ~0.88 which means that this models is an alright fit for this task.
